        ...
        try:
            ...
            if self.settings_manager:
                try:
                    ...
                    max_tokens = summarization_config['max_tokens']
                    temperature = summarization_config['temperature']
                    top_p = summarization_config.get('top_p', 1.0)
            ...
            # 1. Сначала инструкция о формате
            batch_prompt = f"""Обработай следующие {len(texts)} текстов и верни результат в формате JSON массива.
...
"""
            ...
            # 2. Добавляем кастомный промпт или дефолтный
            if custom_prompt:
                batch_prompt += f"{custom_prompt}\n"
            else:
                batch_prompt += f"{self._get_default_prompt(language)}\n"
            
            # 3. Добавляем ограничение по длине
            if summary_length:
                batch_prompt += f"\nЛимит длины: постарайся уложиться примерно в {summary_length} символов, но не жертвуй важными частями ради этого. Лучше слегка превысить лимит, чем потерять суть или вкус текста.\n"
            
            # 4. Добавляем тексты для обработки
