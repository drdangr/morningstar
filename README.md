...
**⚠️ Stage 4 ЧАСТИЧНО ЗАВЕРШЕН** (9 июня 2025) - **Backend API для дайджестов + критический баг**
...
- **✅ Революционная AI-обработка:** семантический анализ + Binary Relevance + метрики + структурированные инструкции v7.2
- **✅ Архитектура "цифровой периодики":** объединяет кураторство и персонализацию

**✅ КРИТИЧЕСКИЙ БАГ ИСПРАВЛЕН (v7.2):**
- **Проблема AI промптов:** N8N workflow неправильно использовал поле "AI Prompt" как тему вместо кастомных инструкций LLM — **РЕШЕНА**
- **Исправленное поведение:** "Description" используется для тем анализа, "AI Prompt" используется для кастомных инструкций LLM
- **Улучшение v7.2:** Структурированные инструкции — каждая инструкция четко привязана к своей теме, избегая "каши" в AI промптах
- **Статус:** Полностью готов к переходу к Stage 5 (Telegram-бот для пользователей)
...
- `/topics` - настроить фильтры по темам
- `/delivery` - настроить время и частоту доставки
- `/feedback` - оценить качество контента
- `/settings` - все персональные настройки
