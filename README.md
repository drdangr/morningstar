...
- **✅ Революционная AI-обработка:** семантический анализ + Binary Relevance + метрики для качественной фильтрации
- **✅ Архитектура "цифровой периодики":** объединяет кураторство и персонализацию

**🚨 КРИТИЧЕСКИЙ БАГ ОБНАРУЖЕН:**
- **Проблема AI промптов:** N8N workflow неправильно использует поле "AI Prompt" как тему вместо кастомных инструкций LLM
- **Текущее поведение:** "AI Prompt" из админ-панели попадает в промпт как тема, вместо "Description"
- **Правильное поведение:** "Description" = тема для анализа, "AI Prompt" = кастомные инструкции для LLM по этой теме
- **Статус:** Требует исправления N8N workflow перед переходом к Stage 5
