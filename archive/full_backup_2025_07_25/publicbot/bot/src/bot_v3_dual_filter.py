...
Логика: (посты из подписанных каналов) ∩ (посты подписанных категорий)
- Backend API интеграция
- Автоподписка при первом старте
- Учет max_posts_per_digest
- Inline-клавиатура
"""

print("🔍 Запуск bot_v3_dual_filter.py...")
...
print("🔧 Все импорты успешны, продолжаем...")
...
async def get_filtered_posts(subscribed_channel_ids, subscribed_categories, max_posts):
    """
    🎯 ПРАВИЛЬНАЯ ДВОЙНАЯ ФИЛЬТРАЦИЯ:
    1. Посты из подписанных каналов
    2. С подписанными категориями
    """
    try:
        params = {
            "bot_id": BOT_ID,
...
async def ensure_auto_subscription(telegram_id):
    """Автоподписка на все категории и каналы при первом старте"""
    try:
...
async def get_categories_keyboard(telegram_id):
    """Клавиатура категорий с статусом подписки"""
...
async def get_channels_keyboard(telegram_id):
    """Клавиатура каналов с статусом подписки"""
...
async def digest_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /digest - формирование персонального дайджеста"""
...
async def main():
    ...
    # Запускаем бота
    try:
        # Запуск без asyncio.run() для избежания конфликта
