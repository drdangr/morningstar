...
- **Логика работы**: 
  ...
  2. При обнаружении → автоматически запускает `trigger_ai_processing`
  3. AI обработка выполняется в фоне через Celery worker
  4. Результаты сохраняются в `processed_data` таблицу
...
- Система самостоятельно обнаруживает новые посты от Userbot
- Автоматически переобрабатывает посты после удаления AI результатов  
- Работает непрерывно при запущенных Docker контейнерах

**📊 Производительность:**
- Проверка необработанных постов: ~20ms (благодаря оптимизированным индексам)
- AI обработка: ~2-3 секунды на пост
- Overhead системы: минимальный (низкий приоритет задач) 
