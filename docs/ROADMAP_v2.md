...
по мере выполнения задачи, не удаляй запись о ней, а видоизменяй запись о ней следующим образом: 

когда задача берется в работу видоизмени ее следующим образом: 
- [ ] **🚧 Задача 1 В РАБОТЕ:** описание задачи 1 
если задача выполнениа приведи ее к виду как в примерах:
...
если с зазачей возникла проблема преобразуй ее к виду: 
...
если пользователь говорит, что день закончен, оставшиеся задачи оформи следующим образом:
...
```
ВАЖНО: не удаляй задачи, просто меняй запись в каждой задаче согласно инструкции: 
- когда задача выполнена модивицируй запись о ней, проставляя галочу в квадратных скобках и помечай, что она выполнена, 
- пока задача в работе - ставь иконку, что она в работе
- если появилсь дополнительная зачада - добавляй ее описание как написано в инструкции выше
```
...
### 📅 День 12. 13 июня 2025: STAGE 1.5 + STAGE 2 (День 1)
**🎯 ЦЕЛЬ ДНЯ:** Завершение STAGE 1.5 + начало STAGE 2 (Python AI Services)

**📋 ПЛАН НА ДЕНЬ:**
#### 1. Завершение STAGE 1.5:
- [ ] 🔄 **STAGE 1.5 Task 2 РЕАЛИЗАЦИЯ** упрощенной архитектуры
  - [ ] 🔄 Реализация простого расписания доставки
  - [ ] 🔄 Создание 3-уровневой системы наследования настроек (Глобальные → Шаблон → Индивидуальные)
  - [x] ✅ Tone of Voice настраивается через AI промпты (решение принято)
  - [ ] 🔄 Автоматическая генерация дайджестов на основе времени последней доставки

- [ ] **🚧 Bulk операции для каналов и категорий**
  - [ ] Создать BotChannelsManager.jsx с bulk операциями
    - [ ] Массовый выбор каналов через чекбоксы
    - [ ] Bulk добавление выбранных каналов к боту
    - [ ] Импорт/экспорт каналов через CSV
    - [ ] Синхронизация списка доступных каналов
    - [ ] Поиск и фильтрация каналов
  - [ ] Создать BotCategoriesManager.jsx с bulk операциями
    - [ ] Шаблоны категорий (Новости, Политика, Бизнес и т.д.)
    - [ ] Bulk добавление категорий с автоматическими приоритетами
    - [ ] Управление приоритетами категорий
    - [ ] Импорт/экспорт категорий через CSV
  - [ ] Интегрировать в BotConfigurationTabs.jsx
    - [ ] 4-колоночный layout (General Settings, Channels, Categories, LLM Settings)
    - [ ] Счетчики каналов и категорий в табах
    - [ ] Сохранение состояния между табами
  - Definition of Done: Bulk операции полностью функциональны в Bot Configuration UI

- [ ] **Frontend UI тестирование упрощенной архитектуры**
  - [ ] Проверка создания ботов с простым расписанием
  - [ ] Тестирование 3-уровневой системы наследования
  - [ ] Валидация настройки Tone of Voice через промпты
  - [ ] Проверка bulk операций для каналов и категорий

- [ ] **Централизованное хранение AI-настроек**
  - [x] ✅ Создана концепция 3-уровневой системы наследования
  - [ ] Реализовать UI для Settings/AI and LLM Settings
    - [ ] Глобальные настройки (system-wide параметры)
    - [ ] Public Bot шаблоны (значения по умолчанию для новых ботов)
    - [ ] Индивидуальные override настройки (в Public Bots edit mode)
  - [ ] API endpoints для bot templates и AI settings overrides
  - [ ] View для эффективных настроек с fallback логикой

#### 2. Начало STAGE 2 - Python AI Services:
- [ ] **Интеграция с Backend API**
  - [ ] **Обработка posts_cache**
    - [ ] Создать модель Post для работы с posts_cache
    - [ ] Реализовать чтение RAW постов из PostgreSQL
    - [ ] Добавить валидацию структуры постов
    - [ ] Тестирование на реальных данных (200 постов)

  - [ ] **Сохранение в processed_data**
    - [ ] Создать модель ProcessedPost
    - [ ] Реализовать сохранение AI анализа
    - [ ] Добавить индексы для быстрого поиска
    - [ ] Тестирование сохранения результатов

- [ ] **Улучшение AI Services**
  - [ ] **Individual AI prompts**
    - [ ] Добавить поддержку кастомных промптов для PublicBot
    - [ ] Реализовать загрузку промптов из БД
    - [ ] Добавить валидацию промптов
    - [ ] Тестирование с разными промптами (включая Tone of Voice)

  - [ ] **Quality metrics и categorization**
    - [ ] Реализовать importance/urgency/significance метрики
    - [ ] Добавить Binary Relevance систему
    - [ ] Интеграция с индивидуальными промптами ботов
    - [ ] Тестирование качества AI анализа
  ...
  - [ ] **Мониторинг и обработка ошибок**
    - [ ] **Error handling и retry логика**
      ...
      - [ ] Тестирование при сбоях API

    - [ ] **Мониторинг через AdminPanel**
      - [ ] Добавить метрики в AI Results Viewer
      - [ ] Реализовать отображение статуса обработки
      - [ ] Добавить графики производительности
      - [ ] Тестирование UI компонентов

**Definition of Done для 13 июня:**
- 🔄 STAGE 1.5:
  - [ ] Bulk операции для каналов и категорий полностью функциональны
  - [ ] 3-уровневая система наследования AI настроек работает
  - [ ] Frontend UI тестирование упрощенной архитектуры пройдено
  - [ ] Централизованное хранение AI-настроек реализовано

- 🔄 STAGE 2 (День 1):
  - [ ] Модели Post и ProcessedPost созданы и протестированы
  - ✅ Интеграция с posts_cache работает стабильно
  - ✅ Кастомные промпты поддерживаются для каждого бота
  - ✅ Реализовано управление тоном голоса
  - ✅ Улучшена обработка ошибок с retry логикой
  - ✅ Добавлен базовый мониторинг в админ-панель
