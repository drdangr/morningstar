...
- [ ] **Решение проблемы постов-сирот**: 
    - [ ] Вариант А: Очистить базу вручную из админ панели
    - [ ] Вариант Б: Удалить 29 постов-сирот как неактуальные
- [x] ✅ **Валидация целостности**: система работает корректно, AI результаты сохраняются в правильном формате

**ГРУППА 2: UI Posts Cache Monitor ревизия** 🟠 **СРЕДНИЙ ПРИОРИТЕТ**
...
**ГРУППА 3: AI Results мультитенантность** 🟡 **НИЗКИЙ ПРИОРИТЕТ**
- [ ] **Завершить фильтр по ботам**: добавить состояние и функции для фильтрации AI результатов по bot_id
- [ ] **Карточки ботов с прогрессом**: отдельные секции для каждого бота с индивидуальным progress bar
- [ ] **AI результаты с указанием бота**: показать в таблице для какого бота каждый результат
- [ ] **Последняя активность по ботам**: когда каждый бот последний раз обрабатывал посты
