...
- [ ] 🔄 **Расширение Posts Cache Monitor для отображения AI результатов**: Единый интерфейс контроля качества AI - **НА ЗАВТРА**
    - [ ] Расширить GET /api/posts/cache с LEFT JOIN к processed_data таблице
    - [ ] Добавить колонки для отображения AI результатов (summary, category, relevance_score)
    - [ ] Создать компактное отображение саммари с возможностью развернуть полный текст
    - [ ] Добавить цветовую индикацию статуса обработки (обработано/не обработано/ошибка)
    - [ ] Реализовать фильтрацию по статусу AI обработки (все/обработанные/необработанные)
    - [ ] Добавить возможность просмотра детальных метрик AI (importance/urgency/significance)
    - [ ] Создать tooltip с полной информацией об AI обработке при hover
    - [ ] Добавить сортировку по метрикам AI и времени обработки
    - Definition of Done: 🔄 **НА ЗАВТРА** - Posts Cache Monitor показывает посты + AI результаты в едином интерфейсе
...
- [ ] 🔄 **Запуск и мониторинг AI обработки**: Проверка качества работы AI сервисов - **НА ЗАВТРА**
    ...
    - Definition of Done: 🔄 **НА ЗАВТРА** - AI сервисы работают стабильно с высоким качеством результатов
