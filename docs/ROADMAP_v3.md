...
**🎯 ЦЕЛЬ ДНЯ:** Заменить реальные AI сервисы на псевдообработку для безопасного end-to-end тестирования без сжигания токенов

**⏱️ ОЦЕНКА ВРЕМЕНИ:** ~5-6 ч разработки + 2 ч тестирования.

**📋 ПЛАН НА ДЕНЬ:**

- [ ] **STAGE 3 Фаза 1: Замена AI-сервисов на псевдообработку**
    - [ ] Изучить текущую структуру AI Services: файлы categorization и summarization
    - [ ] Заменить реальные вызовы OpenAI API на псевдообработку с задержками:
        - [ ] Саммаризация: "саммаризация для поста ID{post_id} для бота id{bot_id}" + delay 2 сек
        - [ ] Категоризация: первая активная категория бота + метрики (importance=post_id, urgency=bot_id, significance=batch_index) + delay 5 сек
    - [ ] Сохранить функционал очередей, асинхронной обработки, проверки дубликатов
    - [ ] Перевести AI Services на использование schemas.py (PostForCategorization, PostForSummarization, ProcessingStatus)
    - **Definition of Done:** AI Services работают с псевдоданными, используют unified схему, сохраняют результаты в БД

- [ ] **STAGE 3 Фаза 2: Создание AI Services Monitor в Frontend**
    - [ ] Создать новый пункт меню "AI Services" вместо старого "AI Results"
    - [ ] Реализовать real-time мониторинг:
        - [ ] Настройки каждого сервиса (модель, токены, промпты)
        - [ ] Статистика обработки (всего/обработано/в работе/очередь)
        - [ ] Текущие задачи с ID постов
        - [ ] Интеграция с Flower данными
    - [ ] Протестировать удаление старого AI Results без поломки системы
    - **Definition of Done:** Новый AI Services Monitor работает, старый AI Results удален

- [ ] **STAGE 3 Фаза 3: End-to-End тестирование псевдообработки**
    - [ ] Протестировать добавление/удаление постов через Frontend
    - [ ] Проверить автоматическое обнаружение новых постов Celery Beat  
    - [ ] Проверить корректность флагов is_categorized/is_summarized
    - [ ] Проверить отображение псевдорезультатов в Posts Cache Monitor
    - [ ] Протестировать устойчивость к параллельным операциям
    - **Definition of Done:** Полный pipeline от поста до отображения работает с псевдоданными

**🧪 КРИТИЧЕСКИ ВАЖНО:** После каждой фазы проводить мини-тестирование, чтобы убедиться что ничего не сломалось
