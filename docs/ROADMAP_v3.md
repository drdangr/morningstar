...
- [ ] **STAGE 2 Дополнительно: Исправление проблем со статусами публичного бота**
    - [ ] Исследовать проблему: почему публичный бот не переключает корректно статус (пауза/настройка/активен)
    - [ ] Найти и исправить проблему в backend логике управления статусами
    - [ ] Проверить валидацию статусов в БД (constraint должен поддерживать все статусы)
    - [ ] Убедиться, что в Post Cache Monitor видно активный бот и его корректный статус
    - [ ] Протестировать переключение статусов через Frontend
    - **Definition of Done:** Статусы ботов корректно сохраняются, отображаются и переключаются
...
- [ ] **STAGE 5 Фаза 1: Восстановление базового функционала Userbot**
    - [x] ✅ Восстановить Telethon сессию и подключение к Telegram API - **РАБОТАЕТ:** Сессия активна, авторизация прошла
    - [x] ✅ Исправить получение системных настроек - **ИСПРАВЛЕНО:** `collection_depth_days` (39 дней) и `max_posts_per_digest` теперь читаются корректно из Backend API
    - [ ] Восстановить базовый сбор постов из настроенных каналов
    - [ ] Восстановить отправку собранных постов в Backend API (/api/posts/cache)
    - [ ] Восстановить систему дедупликации (проверка существующих постов)
    - [ ] Протестировать сбор 10-20 тестовых постов для наполнения БД
    - **Definition of Done:** Userbot собирает новые посты и отправляет их в Backend API без дубликатов
...
- [x] ✅ **STAGE 3 Фаза 2: Создание AI Services Monitor в Frontend - ЗАВЕРШЕН**
    ...
    - [x] ✅ Реализовать real-time мониторинг:
        - [x] ✅ Настройки каждого сервиса (модель, токены, промпты) - **ВЫПОЛНЕНО:** Отображение конфигурации из системных настроек
