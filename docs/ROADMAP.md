...
### Этап 4. Backend API для дайджестов и Docker integration
**Цель:** создать Backend API для сохранения дайджестов из N8N и подготовить полностью докеризованную систему к продакшену.

#### Backend API задачи:
- [ ] Создание Backend API endpoints для сохранения дайджестов из N8N
- [ ] Реализация системы хранения дайджестов в БД (таблица digests)
- [ ] API для получения дайджестов по категориям и пользователям
- [ ] Интеграция N8N "Save to Backend" node с реальным API
- [ ] Система планирования и автоматической генерации дайджестов (cron jobs)
- [ ] API для статистики и аналитики (топ посты, активность каналов)
- [ ] Подготовка архитектуры для пользовательских подписок
...
- Система готова к интеграции с Telegram-ботом в докеризованной среде
- Полная End-to-End цепочка протестирована в Docker окружении

### Этап 5. Публичный Telegram-бот для доставки дайджестов
...
- [ ] Регистрация пользователя (/start) — точка входа с созданием профиля в Backend API
- [ ] Получение списка категорий (/categories) — из Backend API topics
- [ ] Подписка на категории (/subscribe [category]) — управление через Backend API
- [ ] Получение персонализированных дайджестов (/digest) — чтение из Backend API по подпискам
...
- [ ] Красивое форматирование дайджестов — отображение AI summary, метрик, источников
...
- [ ] Админ-команды для управления рассылкой (/admin_broadcast, /admin_stats)
- [ ] Интеграция с Backend API для всех операций с данными

**Критерии готовности этапа (Definition of Done):**
- Полная интеграция с Backend API (пользователи, подписки, дайджесты, фидбек)
- Пользователь может получать персонализированные дайджесты по подпискам
- Автоматическая доставка дайджестов работает стабильно
- Фидбек пользователя сохраняется и влияет на будущие дайджесты
- Красивое UI с кнопками, форматированием и ссылками на источники
...
- [x] ✅ **ЗАВЕРШЕНО:** Интеграция с существующими Categories из админ-панели
- [ ] Создание Backend API endpoints для сохранения дайджестов
- [x] ✅ Обновление userbot для передачи topic metadata
...
- ✅ **AI Processing завершен** - система готова к продакшену
- ✅ **Дайджесты генерируются** - нужен Backend API для сохранения
- ✅ **JSON структура определена** - готова к интеграции с Backend
- 🚀 **Следующий этап:** Backend API для сохранения дайджестов из N8N
